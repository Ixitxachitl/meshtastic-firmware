// Wire.cpp - Arduino TwoWire backed by Zephyr i2c1 (the SF32LB I2C1 controller).
//
...
// Resolve the i2c1 node at compile time. If the overlay has not enabled
// i2c1, this evaluates to a NULL device pointer and every call short-circuits
...
// Wire/Wire1 instances are defined in sifli_arduino.cpp alongside the
...
void TwoWire::begin()
{
    ...
    if (dev == nullptr) {
        LOG_WARN("Wire.begin(): i2c1 not enabled in board DTS");
        return;
    }
    if (!device_is_ready(dev)) {
        LOG_WARN("Wire.begin(): i2c1 device not ready");
        return;
    }
    LOG_INFO("Wire.begin(): i2c1 ready");
}

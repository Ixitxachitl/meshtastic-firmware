// Copied from LovyanGFX 1.2.26 (src/lgfx/v1/panel/Panel_AMOLED.cpp). The only substantive
...
void Panel_AMOLED::writeImage(uint_fast16_t x, uint_fast16_t y, uint_fast16_t w, uint_fast16_t h, pixelcopy_t *param,
                              bool use_dma)
{
    ...
    if (param->transp == pixelcopy_t::NON_TRANSP) {
        if (param->no_convert) {
            ...
            if (!_bus->busy()) {
                ...
                bool nogap = (h == 1) || (param->src_y32_add == 0 &&
                                          ((param->src_bitwidth << pixelcopy_t::FP_SCALE) == (w * param->src_x32_add)));
...
Panel_AMOLED_Framebuffer::Panel_AMOLED_Framebuffer(Panel_AMOLED *panel) : _panel(panel)
{
    assert(_panel);
...
bool Panel_AMOLED_Framebuffer::initFramebuffer(uint_fast16_t w, uint_fast16_t h)
{
    ...
    // Allocation failure is reported by the caller; the original
    // ESP_LOGE has no equivalent here.
    return false;
}

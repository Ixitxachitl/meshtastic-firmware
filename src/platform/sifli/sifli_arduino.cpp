/**
 * sifli_arduino.cpp - Arduino shim implementations for Zephyr/SF32LB52x
 *
 * Provides concrete implementations for Print, HardwareSerial, GPIO, SPI,
 * and String methods declared in Arduino.h / SPI.h, backed by the Zephyr
 * GPIO/SPI drivers. Arduino pin numbers are SF32 PAxx numbers.
 */
...
// Pin mapping: Arduino pin number == SF32 PAxx number. The SoC splits PA0-PA44
// across two GPIO devices, gpioa_00_31 and gpioa_32_44 (which is 0-based at PA32).
// ═════════════════════════════════════════════════════════════════════════════
...
static const struct device *_gpio_dev_for_pin(uint32_t pin, gpio_pin_t *zpin)
{
    if (pin < 32) {
        *zpin = (gpio_pin_t)pin;
        return DEVICE_DT_GET(DT_NODELABEL(gpioa_00_31));
    } else {
        *zpin = (gpio_pin_t)(pin - 32);
        return DEVICE_DT_GET(DT_NODELABEL(gpioa_32_44));
    }
}
...
// adding `-DSIFLI_GPIO_DEBUG` to platformio.ini build_flags. Useful when
...
void digitalWrite(uint32_t pin, uint32_t value)
{
    ...
    if (pin == SX126X_RESET && value == 0) {
        static bool _first_nreset = true;
        if (_first_nreset) {
            _first_nreset = false;
            gpio_pin_t bzpin;
            const struct device *bdev = _gpio_dev_for_pin(SX126X_BUSY, &bzpin);
            if (device_is_ready(bdev)) {
                gpio_pin_configure(bdev, bzpin, GPIO_INPUT);
                int busy_before = gpio_pin_get(bdev, bzpin);
                printk("[sifli] BUSY before first NRESET = %d%s\n", busy_before,
...
#ifdef SIFLI_GPIO_DEBUG
    if ((pin == SX126X_CS || pin == SX126X_RESET) && _gpio_log_count < GPIO_LOG_MAX) {
...
#ifdef SIFLI_GPIO_DEBUG
    // Log BUSY state changes + periodic updates for the first 10 seconds
    if (pin == SX126X_BUSY) {
...
// ─── attachInterrupt - supports up to SIFLI_MAX_IRQS pins ────────────────
#define SIFLI_MAX_IRQS 8
...
static _PinIrq _irq_table[SIFLI_MAX_IRQS];
...
void attachInterrupt(uint32_t pin, voidFuncPtr cb, int mode)
{
    ...
    for (int i = 0; i < SIFLI_MAX_IRQS; i++) {
...
// SPI1 carries both the SX1262 and the TF card; each drives its own CS.
...
static const struct device *_spi1(void)
{
    static const struct device *dev = nullptr;
    if (!dev) {
        dev = DEVICE_DT_GET(DT_NODELABEL(spi1));
        if (!device_is_ready(dev)) {
            printk("[sifli] spi1 NOT READY\n");
            dev = nullptr;
        } else {
            printk("[sifli] spi1 ready\n");
        }
    }
    return dev;
}
...
// 4 MHz is a conservative bring-up speed; the SX1262 accepts up to 16 MHz.
static const struct spi_config _spi1_cfg = {
    .frequency = 4000000U,
...
uint8_t SPIClass::transfer(uint8_t data)
{
    ...
    int ret = spi_transceive(dev, &_spi1_cfg, &tx_set, &rx_set);
...
uint16_t SPIClass::transfer16(uint16_t data)
{
    ...
    struct spi_buf_set tx_set = {.buffers = &tx_buf, .count = 1};
    struct spi_buf_set rx_set = {.buffers = &rx_buf, .count = 1};
    spi_transceive(dev, &_spi1_cfg, &tx_set, &rx_set);
...
void SPIClass::transferBytes(const uint8_t *tx, uint8_t *rx, uint32_t count)
{
    ...
    spi_transceive(dev, &_spi1_cfg, &tx_set, rx ? &rx_set : nullptr);
}

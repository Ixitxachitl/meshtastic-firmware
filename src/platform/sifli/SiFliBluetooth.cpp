// SiFliBluetooth.cpp - Zephyr BLE GATT peripheral for Meshtastic nRF54L15
//
...
// Call this from sifliSetup() (main Zephyr thread, CONFIG_MAIN_STACK_SIZE)
// so that by the time SiFliBluetooth::setup() runs from PowerFSMThread,
...
void sifli_bt_preinit()
{
    if (!bt_initialized) {
        ...
        // LittleFS is already mounted by fsInit() before sifliSetup() runs.
...
static bool sifli_bt_init_common()
{
...
void SiFliBluetooth::setup()
{
    LOG_INFO("SiFliBluetooth::setup()");
    if (!sifli_bt_init_common()) {
        return;
    }
    ble_enabled = true;
    start_advertising();
...
void SiFliBluetooth::shutdown()
{
    LOG_INFO("SiFliBluetooth::shutdown()");
    ble_enabled = false;
    stop_advertising();
...
void SiFliBluetooth::startDisabled()
{
...
void SiFliBluetooth::resumeAdvertising()
{
    ble_enabled = true;
    start_advertising();
}

void SiFliBluetooth::clearBonds()
{
    LOG_INFO("BLE clear bonds");
    bt_unpair(BT_ID_DEFAULT, BT_ADDR_LE_ANY);
}

bool SiFliBluetooth::isConnected()
{
    return active_conn != nullptr;
}

int SiFliBluetooth::getRssi()
{
    return 0; // TODO: Zephyr has no direct bt_conn_get_rssi; use HCI RSSI read
              // command
}

void SiFliBluetooth::sendLog(const uint8_t *logMessage, size_t length)
{

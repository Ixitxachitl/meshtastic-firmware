...
FS_LITTLEFS_DECLARE_DEFAULT_CONFIG(sifli_lfs_data);

static struct fs_mount_t _lfs_mnt = {
    .type = FS_LITTLEFS,
    .mnt_point = SIFLI_FS_MOUNT,
    .fs_data = &sifli_lfs_data,
...
void InternalFileSystem::toabs(const char *rel, char *abs, size_t abssz)
{
    ...
    if (rel[0] == '/' && rel[1] == '\0') {
        ...
        snprintf(abs, abssz, "%s%s", SIFLI_FS_MOUNT, rel);
    } else {
        snprintf(abs, abssz, "%s/%s", SIFLI_FS_MOUNT, rel);
    }
}
...
File InternalFileSystem::open(const char *path, const char *mode)
{
    ...
    char abs[SIFLI_FS_PATHLEN];
    toabs(path, abs, sizeof(abs));

    auto s = std::make_shared<SiFliFileState>();
    if (!s)
        return File();
...
bool InternalFileSystem::rename(const char *from, const char *to)
{
    if (!_mounted)
        return false;
    char absfrom[SIFLI_FS_PATHLEN], absto[SIFLI_FS_PATHLEN];
...
File File::openNextFile()
{
    ...
    char childabs[SIFLI_FS_PATHLEN];

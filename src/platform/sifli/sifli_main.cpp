/*
 * sifli_main.cpp - Zephyr entry point for Meshtastic nRF54L15 port
 *
...
extern "C" void k_sys_fatal_error_handler(unsigned int reason, const struct arch_esf *esf)
{
    ...
    printk("[sifli] FATAL reason=%u pc=0x%08x lr=0x%08x cfsr=0x%08x\n", reason, saved_crash.pc, saved_crash.lr, saved_crash.cfsr);
    ...
    printk("[sifli] PSP=0x%08x - stack walk:\n", psp);
    ...
    const uintptr_t SRAM_END = 0x2007FC00UL; // 511 KB, up to the shared window
    if (psp < SRAM_START || psp >= SRAM_END || (psp & 0x3U) != 0) {
        printk("[sifli]   PSP out of SRAM range or unaligned, skipping walk\n");
    } else {
        ...
        for (int i = 0; i < max_words; i++) {
            uint32_t v = sp[i];
            // Code XIPs from the MPI2 NOR window at 0x12000000.
            if (v >= 0x12000000UL && v < 0x20000000UL && (v & 1)) {
                printk("[sifli]   sp[%d]=0x%08x (code)\n", i, v);
            }
        }
    }
...
int main(void)
{
    ...
    printk("[sifli] Reset cause: 0x%08x\n", reset_cause);

    if (saved_crash.magic == CRASH_MAGIC) {
        printk("[sifli] Prev crash: reason=%u pc=0x%08x lr=0x%08x psp=0x%08x xpsr=0x%08x cfsr=0x%08x\n", saved_crash.reason,

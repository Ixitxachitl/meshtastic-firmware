...
// frameCount lets this resolve the backwards wrap off frame 0, whose incoming frame is the *last*
// one. Callers that identify themselves by index must pass it; the 0 default keeps the old, wrong
// answer rather than a plausible-looking one, so a missing argument shows up as the same blank
// frame rather than as silently drawing the wrong content.
static inline uint8_t frameIndexFor(const OLEDDisplayUiState *state, size_t frameCount = 0)
{
    ...
    if (state->frameState == IN_TRANSITION && state->transitionFrameRelationship == TransitionRelationship_INCOMING) {
        if (state->frameTransitionDirection < 0) {
            ...
            // special case: nextFrame() already stores the wrapped index in transitionFrameTarget,
            // which is why last-to-first worked and first-to-last did not.

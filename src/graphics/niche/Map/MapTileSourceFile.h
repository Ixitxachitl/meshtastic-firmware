...
Holds NO per-tile index in RAM - just that small rectangle table (see ranges_ below, one entry per
baked region), so a tile's position in the index is a table scan plus
...
class FileTileSource : public TileSource
{
  ...
  private:
    ...
    File file_;
    // Sized to the file's own range count rather than a fixed kTileBlobMaxZoomRanges array: the
    // wire format's u8 count allows 255 ranges (2.5KB held permanently, on every build carrying
    // this source, for a table a real bake fills a handful of entries of).
    ...
    // ranges may share a zoom. Small enough (one byte per plausible zoom) to keep inline.

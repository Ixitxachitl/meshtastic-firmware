...
// Simple 2D compass renderer for memory-constrained devices
// This is the develop branch version without 3D sphere rendering
// Only compile this file when USE_SIMPLE_COMPASS_RENDERER is explicitly defined
#ifdef USE_SIMPLE_COMPASS_RENDERER

#include "CompassRenderer.h"
#include "Math3D.h" // For Quat type
#include "NodeDB.h"
...
void drawCompassNorth(OLEDDisplay *display, int16_t compassX, int16_t compassY, float myHeading, int16_t radius)
{
    ...
    if (isHighResolution()) {
        ...
        display->fillRect(north.x - 8, north.y - 1, display->getStringWidth("N") + 3, FONT_HEIGHT_TINY - 6);
#else
        display->fillRect(north.x - 8, north.y - 1, display->getStringWidth("N") + 3, FONT_HEIGHT_SMALL - 6);
#endif
    } else {
#if defined(USE_TINY_FONT)
        display->fillRect(north.x - 4, north.y - 1, display->getStringWidth("N") + 2, FONT_HEIGHT_TINY - 6);
#else
...
// Stub implementations for 3D functions (not available in simple renderer)
// Quat is defined in Math3D.h at global scope
void setTopDownView(bool enable)
{
    // No-op in simple renderer
}
...
{
    // Fall back to simple circle (ignore attitude)
    display->drawCircle(cx, cy, radius);
}
...
void drawCenterNeedle3D(OLEDDisplay *display, int16_t cx, int16_t cy, uint16_t radius, const ::Quat &attitude, float bearingRad,
                        float elevRad)
{
    // Fall back to simple arrow (ignore attitude and elevation)
...
#endif // USE_SIMPLE_COMPASS_RENDERER
#endif // HAS_SCREEN
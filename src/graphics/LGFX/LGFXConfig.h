...
class LGFXConfig : public lgfx::LGFX_Device
{
    ...
    lgfx::IBus *_bus_instance = nullptr;
    lgfx::ILight *_light_instance = nullptr;
  ...
  public:
...
#ifdef ARCH_PORTDUINO
#if defined(SDL_h_)
        if (auto *panel = dynamic_cast<lgfx::Panel_sdl *>(_panel_instance)) {
            lgfx::v1::touch_point_t point;
            if (panel->getTouchRaw(&point, 1)) {
                *x = point.x;
                *y = point.y;
                return true;
            }
    ...
    LGFXConfig(const DisplayDriverConfig &config)
    {
        ...
        const bool useSimWindow = config._panel.type &&
                                  (strcasecmp(config._panel.type, "X11") == 0 || strcasecmp(config._panel.type, "SDL") == 0);
        ...
        { // Configure settings for touch control.
            if (config._touch.type && strcasecmp(config._touch.type, "NOTOUCH") != 0) {
                ...
                _touch_instance = new lgfx::Touch_sdl(static_cast<lgfx::Panel_sdl *>(_panel_instance));
                _panel_instance->setTouch(_touch_instance);
            }
#endif
#endif

...
#endif
#if defined(SENSECAP_INDICATOR)
    // The buzzer hangs off the RP2040 co-processor: the melody crosses the interdevice link
    // whole and is played there, so this returns instead of pacing the notes itself.
    indicatorBuzzer.playTones(tone_durations, (size_t)size);
    return;
#endif

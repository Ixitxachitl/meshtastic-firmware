...
static size_t tonesToRtttl(char *out, size_t cap, const ToneDuration *td, int n)
{
    if (!out || cap == 0 || !td || n <= 0)
        return 0;
    out[0] = '\0';

    // b=240 makes a whole note 1000 ms, so the kDur table above is in real
    // milliseconds and the denominators come straight out of it.
    int wrote = snprintf(out, cap, "sys:d=4,o=4,b=240:");
    if (wrote <= 0 || (size_t)wrote >= cap) {
        // Header alone did not fit, so nothing usable was produced.
        out[0] = '\0';
        return 0;
    }
    ...
    return notesWritten ? pos : 0;
}

// The one buffer we build melodies in. Static for two reasons: AudioFileSourcePROGMEM
// keeps a raw pointer to whatever we hand beginRttl(), and the boot melody is requested
// from setup() before audioThread exists - buzzOnAudioThreadReady() replays it from here
// once main() has created the thread.
static char rtttlBuf[384];
static bool rtttlPending = false;

void buzzOnAudioThreadReady()
{
    if (!audioThread || !rtttlPending)
        return;
    rtttlPending = false;
    audioThread->beginRttl(rtttlBuf, std::strlen(rtttlBuf));
}

static void playTonesI2S(const ToneDuration *tone_durations, int size)
{
    if (tonesToRtttl(rtttlBuf, sizeof(rtttlBuf), tone_durations, size) == 0)
        return;
    if (!audioThread) {
        rtttlPending = true; // main() will hand it over via buzzOnAudioThreadReady()
        return;
    }
    rtttlPending = false;
    audioThread->beginRttl(rtttlBuf, std::strlen(rtttlBuf));
...
void playTones(const ToneDuration *tone_durations, int size)
{
    ...
    if (moduleConfig.external_notification.use_i2s_as_buzzer) {
        playTonesI2S(tone_durations, size);
        return;
    }
#endif
#if defined(HAS_I2S_SPEAKER_NRF52)

...
ProcessMessage Breakout::handleReceived(const meshtastic_MeshPacket &mp)
{
    ...
    if (!pb_decode(&stream, meshtastic_GameLeaderboard_fields, &lb))
        return ProcessMessage::CONTINUE;
    if (lb.game != meshtastic_GameType_GAME_BREAKOUT || lb.entries_count == 0)
        return ProcessMessage::CONTINUE;
    ...
    for (pb_size_t i = 0; i < lb.entries_count; i++) {
        ...
        if (rank >= 0) {
            changed = true;
            LOG_INFO("Breakout: remote score %lu from 0x%08x placed at rank %d", static_cast<unsigned long>(e.score), nodeNum,
                     rank + 1);
        }
    }
    if (changed)
...
int32_t Breakout::meshTick(GamesModule &host)
{
...
void Breakout::broadcastAllScores(GamesModule &host)
{
#if GAME_DEMO_MODE
    return;
#endif
    ...
    if (pb_encode(&stream, meshtastic_GameLeaderboard_fields, &lb)) {
        ...
        LOG_INFO("Breakout: broadcast table (%u entries)", lb.entries_count);
    } else {
        LOG_WARN("Breakout: pb_encode table failed");

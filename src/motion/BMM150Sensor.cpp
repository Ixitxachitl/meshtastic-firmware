...
void BMM150Sensor::calReset_() {
  calActive_ = false; calEndMs_ = 0; calSamples_ = 0;
  min_[0]=min_[1]=min_[2]=+1e9f; max_[0]=max_[1]=max_[2]=-1e9f;
}
...
void BMM150Sensor::calPush_(float mx, float my, float mz) {
  if (mx < min_[0]) min_[0] = mx;
  if (mx > max_[0]) max_[0] = mx;

  if (my < min_[1]) min_[1] = my;
  if (my > max_[1]) max_[1] = my;

  if (mz < min_[2]) min_[2] = mz;
  if (mz > max_[2]) max_[2] = mz;

  ++calSamples_;
}

void BMM150Sensor::calSolve_() {
  for (int i=0;i<3;++i) cal_.offset[i] = 0.5f * (max_[i] + min_[i]);
  float r[3] = { 0.5f*(max_[0]-min_[0]), 0.5f*(max_[1]-min_[1]), 0.5f*(max_[2]-min_[2]) };
  float rmean = (r[0]+r[1]+r[2]) / 3.0f;
  for (int i=0;i<3;++i) cal_.scale[i] = (r[i] > 1e-3f) ? (rmean / r[i]) : 1.0f;
  cal_.valid = true;
  calActive_ = false;
...
int32_t BMM150Sensor::runOnce()
{
    ...
    const float my2 =  mxc * sinf(roll) * sinf(pitch)
                     + myc * cosf(roll)
                     - mzc * sinf(roll) * cosf(pitch);

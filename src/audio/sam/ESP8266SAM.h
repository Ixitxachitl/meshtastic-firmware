...
/**
 * Minimal sink interface, replacing ESP8266Audio's AudioOutput.
 *
 * Vendored so SAM no longer drags in ESP8266Audio, whose bundled libmad collides at link
 * time with the copy inside BackgroundAudio. These are the only methods Say() ever used.
 */
class SamAudioOut {
  public:
    virtual ~SamAudioOut() {}
    ...
    /// Return false to abandon the utterance; Say() stops rendering.
...
public:
  ...
  bool Say(SamAudioOut *out, const char *str);
  bool Say_P(SamAudioOut *out, const char *str) {
    char ram[256];

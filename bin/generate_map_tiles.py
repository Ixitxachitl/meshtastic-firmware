...
def fetch_tile(
    session: requests.Session, style: str, api_key: str, z: int, x: int, y: int, retries: int = 3
...
def bake_tile(session: requests.Session, style: str, api_key: str, z: int, x: int, y: int, threshold: int,
              invert: bool) -> tuple[BakedTile, int]:
    img, download_bytes = fetch_tile(session, style, api_key, z, x, y)
    packed = threshold_tile(img, threshold, invert)
    if packed == b"":
        return BakedTile(z, x, y, KIND_WHITE, b""), download_bytes
    if packed == b"\xff":
        return BakedTile(z, x, y, KIND_BLACK, b""), download_bytes
    ...
    return BakedTile(z, x, y, KIND_LZ4, compressed), download_bytes
...
def render_progress(done: int, total: int, elapsed: float, download_bytes: int) -> None:
    ...
    sys.stdout.write(f"\r[{bar}] {done}/{total} ({pct * 100:5.1f}%)  {tiles_per_sec:5.1f} tiles/s  {rate_str}   ")
    sys.stdout.flush()
...
def write_blob(tiles: list[BakedTile], out_path: Path) -> None:
    ...
    for t in tiles:
        offset = len(payload)
        payload += t.payload
        index += struct.pack("<BHHBIH", t.zoom, t.tx, t.ty, t.kind, offset, len(t.payload))

    with open(out_path, "wb") as f:
        f.write(b"MTLB")
        f.write(struct.pack("<I", len(tiles)))  # u32: z8 alone is 65536 tiles, over u16's range
        f.write(index)
        f.write(payload)
...
def main() -> int:
    ...
    ap.add_argument("--style", default="toner-v2", help="MapTiler style id (default: toner-v2)")
    ap.add_argument("--zooms", default="0-2", help="Zoom range, e.g. '0-4' or a single level '3'")
    ap.add_argument("--threshold", type=int, default=128, help="Grayscale threshold (0-255) for 1-bit conversion")
    ap.add_argument("--invert", action="store_true", help="Invert black/white (this style renders dark ocean by default)")
    ap.add_argument("--out", type=Path, default=Path("world_blob.bin"), help="Output blob path")
    ap.add_argument("--emit-header", type=Path, default=None, help="Also emit a MapTile.h-compatible header (small datasets only)")
    ap.add_argument("--max-tiles", type=int, default=None, help="Safety cap on total tiles fetched (for test runs)")
    ap.add_argument("--workers", type=int, default=16, help="Concurrent fetch workers (default 16)")
    ap.add_argument(
        "--cache",
        type=Path,
        default=None,
        ...
        "in-flight tiles, not the whole run.",
    )
    ap.add_argument(
    ...
    coords = [(z, tx, ty) for z in range(z_lo, z_hi + 1) for ty in range(1 << z) for tx in range(1 << z)]
    if args.max_tiles is not None:
        coords = coords[: args.max_tiles]
    total_planned = len(coords)

    cache_path = args.cache if args.cache is not None else args.out.with_suffix(args.out.suffix + ".cache")
    ...
    raw_total = sum(TILE_SIZE * TILE_SIZE // 8 for t in tiles_by_coord.values() if t.kind == KIND_LZ4)
    compressed_total = sum(len(t.payload) for t in tiles_by_coord.values() if t.kind == KIND_LZ4)
    fetched = 0
    download_bytes = 0
    ...
    def worker(coord):
        ...
        try:
            t, n_bytes = bake_tile(thread_local, args.style, args.api_key, z, tx, ty, args.threshold, args.invert)
            return coord, t, n_bytes, None
        except Exception as exc:  # noqa: BLE001 - report it, don't crash the whole pool
            return coord, None, 0, exc

    _thread_state = threading.local()
    cache_path.parent.mkdir(parents=True, exist_ok=True)
    with open(cache_path, "ab") as cache_file, ThreadPoolExecutor(max_workers=args.workers) as pool:
        futures = {pool.submit(worker, c): c for c in todo}
        for future in as_completed(futures):
            if aborted:
                future.cancel()
                continue
            coord, t, n_bytes, exc = future.result()
            if exc is not None:
                consecutive_failures += 1
                sys.stdout.write("\n")
                print(f"  FAILED {coord}: {exc} ({consecutive_failures}/{args.max_consecutive_failures} consecutive)")
            ...
            with lock:
                ...
                if now - last_render > 0.2 or fetched == len(todo):
                    render_progress(resumed + fetched, total_planned, now - start_time, download_bytes)
                    last_render = now
...
def print_report(tiles, kind_counts, raw_total, compressed_total, out_path: Path) -> None:
    ...
    if raw_total:
        print(f"LZ4 tiles: {raw_total} bytes raw -> {compressed_total} bytes compressed "
              f"({100.0 * compressed_total / raw_total:.1f}%)")
    print(f"Output blob: {out_path} ({blob_size} bytes, {blob_size / 1024:.1f} KiB, {blob_size / 1024 / 1024:.2f} MiB)")
    ...
    for z in sorted(by_zoom):
        ...
        payload_bytes = sum(len(t.payload) for t in zt) + 13 * len(zt)  # + per-tile index overhead
        print(f"  z{z}: {len(zt)} tiles, {free} free ({100.0*free/len(zt):.0f}%), "
